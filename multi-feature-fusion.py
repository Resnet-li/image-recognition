...
for i, layer in enumerate(conv_base.layers):
    if "batch_normalization" in layer.name:
        conv_base.layers[i] = GroupNormalization(groups=32, axis=-1, epsilon=0.00001)       

for i, layer in enumerate(conv_base2.layers):
    if "batch_normalization" in layer.name:
        conv_base2.layers[i] = GroupNormalization(groups=32, axis=-1, epsilon=0.00001)        

for i, layer in enumerate(conv_base3.layers):
    if "batch_normalization" in layer.name:
        conv_base3.layers[i] = GroupNormalization(groups=32, axis=-1, epsilon=0.00001)
...
model.compile(optimizer=SGD(lr=1e-2),
              ...
              metrics=['accuracy'])
lr_reduce = ReduceLROnPlateau(monitor='val_loss', factor=0.8, patience=60,  min_lr = 0.00000001)
